...
/**
 __NOT__ foolproof.
 Actually, there are many cases that are not working.
 See HappnXib2Lokalise. */
struct HappnXib2Std {
	
	/* Guarantees on return value:
	 *    - Each array will contains at least one element;
	 *    - Each elements inside an array will be kind of the same subclass of LocValueTransformer. */
...
#if canImport(os)
					Conf.oslog.flatMap{ os_log("Got a printf-style replacement AND a std loc entry action (%{public}@)", log: $0, type: .info, stdLocEntryAction) }
#endif
					Conf.logger?.warning("Got a printf-style replacement AND a std loc entry action (\(stdLocEntryAction))")
				}
	...
	private static func simpleReplacementForStdConversion(from xibLocValues: [XibRefLocFile.Language: XibRefLocFile.Value], idx: Int, formatSpecifier: String, tokens: (String, String)?) -> String {
		...
		/* When stripping whitespaces and newlines from r in the line below, we assume whitespaces and newlines are all represented on a single unicode scalar. */
...
#if canImport(os)
			Conf.oslog.flatMap{ os_log("Cannot get name of replacement (tokens %{public}@ and %{public}@) with values %@", log: $0, type: .info, leftToken, rightToken, xibLocValues) }
#endif
			Conf.logger?.warning("Cannot get name of replacement (tokens \(leftToken) and \(rightToken)) with values \(xibLocValues)")
