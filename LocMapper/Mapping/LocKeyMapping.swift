...
public final class LocKeyMapping {
	...
	/**
	 Compute whether the given transform is valid (do not check for existence of keys for mapping components though). */
	...
	/**
	 ...
	 If the string representation is invalid (invalid JSON, etc.), a fully inited object is returned with nil components. */
	public convenience init?(stringRepresentation: String) {
		...
		guard
			...
			{
#if canImport(os)
				Conf.oslog.flatMap{ os_log("Invalid mapping; cannot serialize JSON string: \"%@\"", log: $0, type: .info, stringRepresentation) }
#endif
				Conf.logger?.warning("Invalid mapping; cannot serialize JSON string: \"\(stringRepresentation)\"")
...
#if canImport(os)
			Conf.oslog.flatMap{ os_log("Invalid mapping; cannot convert string to array of dictionary: \"%@\"", log: $0, type: .info, stringRepresentation) }
#endif
			Conf.logger?.warning("Invalid mapping; cannot convert string to array of dictionary: \"\(stringRepresentation)\"")
			self.init(components: nil, stringRepresentation: stringRepresentation)
			return
		}
